...
public class StudentCampService {
	...
	public static void registerAsAttendee(){
		Student user = (Student)AuthData.getCurrentUser();
		Camp camp = AuthData.getCurrentCamp();
		ArrayList<String> registeredCamp1 = user.getRegisteredCamp();

		for(String campname: registeredCamp1){
			Camp camp1 = DatabaseService.getCamp(campname);
			if(checkClash(camp1, camp)){
				ColouredTextPrinter.printRed("Date clash!! Unable to registered.");
				return;
			}
		}
	...
	public static void registerAsCommittee(String position){
		...
		for(String campname: registeredCamp){
			Camp camp1 = DatabaseService.getCamp(campname);
			if(checkClash(camp1, camp)){
				ColouredTextPrinter.printRed("Date clash!! Unable to registered.");
				return;
	...
	public static void withdrawCamp(String campName, Student student, boolean isComittee){
		...
		ArrayList<String> withdrawCamp = new ArrayList<>();
